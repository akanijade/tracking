...
def constructBayesNet(gameState: hunters.GameState):
    ...
    # add var -> values in dictionary
    for var in variables[:3]: # [PAC, GHOST0, GHOST1]
        variableDomainsDict[var] = positions.copy() # all the positions on the board
    
    for obs in variables[3:]:  # [OBS0, OBS1]
        values = set() # holds unique values
        
        for pacPos in positions: # for all possible positons of pacman on the board
            for ghostPos in positions: # for all possible positions of a ghost on the board
                dist_ = manhattanDistance(pacPos, ghostPos)
                # Manhattan distances of Pacman to ghosts ± noise, and are non-negative.
                if dist_ + MAX_NOISE >= 0: # Manhattan distances of Pacman to ghosts + MAX_NOISE is non-negative.
                    values.add(dist_ + MAX_NOISE)
                if dist_ - MAX_NOISE >= 0: # Manhattan distances of Pacman to ghosts - MAX_NOISE is non-negative.
                    values.add(dist_ - MAX_NOISE)
                    
        # set each `variableDomainsDict[var] = values`, where `values` is a LIST of the possible assignments to `var`.
        # sort the list for the autograder
        variableDomainsDict[obs] = list(sorted(values))
    # raiseNotDefined()
    # raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        for factor in eliminationOrder:
            currentFactors, joined = joinFactorsByVariable(currentFactors, factor)
            if len(joined.unconditionedVariables()) > 1:
                currentFactors.append(eliminate(joined, factor))

        currentFactors = joinFactors(currentFactors)
        currentFactors = normalize(currentFactors)
...
class DiscreteDistribution(dict):
    ...
    def normalize(self):
        ...
        "*** YOUR CODE HERE ***"
        summed = self.total()
        if self.keys() is None or summed == 0:
            return
        for key in self:
            self[key] = self[key] / summed
        # raiseNotDefined()
        "*** END YOUR CODE HERE ***"
    ...
    def sample(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        probability = self.total() * random.random()
        cumSum = .0
        for key in self:
            if cumSum <= probability < cumSum + self[key]:
                return key
            cumSum = cumSum + self[key]
        # raiseNotDefined()
        "*** END YOUR CODE HERE ***"
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for ghostPosition in self.allPositions:
            self.beliefs[ghostPosition] = self.beliefs[ghostPosition] * self.getObservationProb(observation, pacPosition, ghostPosition, jailPosition)
        # raiseNotDefined()
        "*** END YOUR CODE HERE ***"
        self.beliefs.normalize()
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for pos in self.allPositions:
            distance = self.getPositionDistribution(gameState, pos)
            for pos2 in distance:
                newProb[pos2] = newProb[pos2] + (distance[pos2] * self.beliefs[pos])
        newProb.normalize()
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        self.particles = []
        "*** YOUR CODE HERE ***"
        # particles_posi = self.numParticles / len(self.legalPositions)
        # #self.particles = []
        # for posi in self.legalPositions:
        #     for i in range(particles_posi):
        #         self.particles.append(posi)
        total = len(self.legalPositions)
        self.beliefs = util.Counter()
        for i in range(self.numParticles):
            posi = self.legalPositions[i % total]
            self.particles.append(posi)
            self.beliefs[posi] += 1
        self.beliefs.normalize()
        #raiseNotDefined()
